...
pooldel(Pool *p, Bhdr *t)
{
	...
	if(t->nextnode != nil && p->root != t){
	
	//find previous node
	n = p->root;
	while(n->nextnode != t && n->nextnode != nil){
		n= n->nextnode;
	}
	...
	//set the node to point to rest of list;
...
poolmax(Pool *p)
{
	...
	t = p->root;
	if(t != nil){
		//have to compare entire list instead of biggest right node
		//no structure of inserting into linkedlist of size
		while(t!= nil){
